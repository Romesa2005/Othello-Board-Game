...
public class OthelloBoard {
	...
	public char get(int row, int col) {
		...
		if (this.board[row][col] == 'X'){return P1;}
		else if (this.board[row][col] == 'O') {return P2;
		}
	}
	return EMPTY;}
	...
	private char alternation(int row, int col, int drow, int dcol) {
		...
		char newvar = this.board[row][col];
		int i = row;
		int j = col;
		if (drow ==0 && dcol ==0) {
			return EMPTY;
		}
		while(validCoordinate(i,j) && newvar != EMPTY && newvar == this.board[i][j] && i<this.dim && j<this.dim && i>=0 && j>=0) {
			i=i+drow;
			j=j+dcol;
		}
		if (!validCoordinate(i,j) || this.board[i][j] == newvar){return EMPTY;}
		else{return this.board[i][j];}
		}
	/**
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		if (alternation(row,col,drow,dcol) ==EMPTY){
		return -1;}
		char newvar = this.board[row][col];
		int i = row;
		int j = col;
		int count =0;
		while(newvar == this.board[i][j] && i<this.dim && j<this.dim && i>=0 && j>=0) {
	...
	private char hasMove(int row, int col, int drow, int dcol) {
		...
		if (this.board[row][col] == EMPTY){
			if (row+drow>this.dim || col+dcol>this.dim){
				return EMPTY;
			}
	...
	public char hasMove() {
		...
		for (int row = 0; row < this.dim; row++) {
			for (int col = 0; col < this.dim; col++) {
				if (hasMove(row,col,0,1) == P1 || hasMove(row,col,0,-1) == P1||
						hasMove(row,col,1,0) == P1 ||hasMove(row,col,-1,0) == P1 ||
						hasMove(row,col,1,1) == P1 ||hasMove(row,col,1,-1) == P1 ||
						hasMove(row,col,-1,-1) == P1 ||hasMove(row,col,-1,1) == P1 )
				{player1 = true;}
				if (hasMove(row,col,0,1) == P2 || hasMove(row,col,0,-1) == P2||
						hasMove(row,col,1,0) == P2 ||hasMove(row,col,-1,0) == P2 ||
						hasMove(row,col,1,1) == P2 ||hasMove(row,col,1,-1) == P2 ||
						hasMove(row,col,-1,-1) == P2 ||hasMove(row,col,-1,1) == P2) {
				player2 = true;
				}}}
			if (player1 && player2){
				return BOTH;
			} else if (player1) {return P1;
			} else if (player2) {return P2;
			}
			else {return EMPTY;}
	...
	/**
	I made this helper function for Random Player because I needed a method that only checked
	if a player had move but didn't actually move or flip.
	* @param row    starting row, in {0,...,dim-1} (typically {0,...,7})
	* @param col    starting col, in {0,...,dim-1} (typically {0,...,7})
	* @param player P1 or P2
	 * @return true if player can make a move at the given coordinates
	 */
	public boolean helperforrandom(int row, int col, char player) {
	...
	/**
	 * I made this helper function for Greedy Player because I needed a method that only returned
	 * the number of tokens that may flip at the given coordinated
	 *@param row    starting row, in {0,...,dim-1} (typically {0,...,7})
	 *@param col    starting col, in {0,...,dim-1} (typically {0,...,7})
	 * @param player P1 or P2
	 * @return the number of tokens that may flip
	 */


	public int helperforgreedy(int row, int col,char player) {
		int count = 0;
		...
		for (int drow = -1; drow <= 1; drow++) {
			for (int dcol = -1; dcol <= 1; dcol++){
				if (hasMove(row,col,drow,dcol) == player )
				{
					char newvar = this.board[row+drow][col+dcol];
					int i = row+drow;
					int j = col+dcol;
					while(newvar == this.board[i][j] && i<this.dim && j<this.dim && i>=0 && j>=0) {
						i=i+drow;
	...
	public static void main(String[] args) {
		...
		System.out.println("Trying to move to (2,4) move=" + ob.move(2, 4, P2));
		System.out.println(ob.toString());
	}
}
